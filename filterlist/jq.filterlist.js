    ...
    $.fn.filterList = function (opts) {
        ...
        if (this[0].tagName == 'TABLE') {
            ...
            for (var i = 0; i < body.length; i++) {
                filterCache[body[i].id] = new filterList(body[i], opts, input, closer);
            }
        }
        else {
            for (var i = 0; i < this.length; i++) {
                filterCache[this[i].id] = new filterList(this[i], opts, input, closer);
            }
        }
    };


    var filterList = function (elem, opts, input, closer) {
        this.options = [];
        this.ele = ele = $(elem);
